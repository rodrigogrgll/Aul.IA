...
# --- CONFIGURACIÓN DE PÁGINA (CORRECCIÓN 1) ---
...
# --- (CORRECCIÓN 1: AÑADIMOS EL "TELÉFONO" Y EL MODELO) ---
# Configura el modelo que vamos a usar
model = genai.GenerativeModel('models/gemini-2.5-pro') 

def llamar_a_gemini(prompt_usuario):
    """
    Envía un prompt a Gemini y devuelve la respuesta como texto.
    ...
    try:
        ...
        # Manejo de errores (ej. si la API falla, contenido bloqueado, etc.)
...
st.markdown(
    ...
    <svg class='background-svg' viewBox='0 0 1000 1000' preserveAspectRatio='xMidYMid slice' xmlns='http://www.w3.org/2000/svg'>
        ...
        <circle cx='100' cy='100' class='node-circle'/>
        ...
        <path d='M60 70 L60 80 A10 10 0 0 0 70 90 A10 10 0 0 0 80 80 L80 70 A10 10 0 0 0 70 60 A10 10 0 0 0 60 70 Z M68 90 L72 90 M68 93 L72 93' transform='scale(0.8) translate(700 800)' class='stroke-style'/>
...
if 'plan_de_foco' not in st.session_state:
    st.session_state.plan_de_foco = None # Almacenará el Plan de Foco (Prompt 1)

if 'sa_generada' not in st.session_state:
    st.session_state.sa_generada = None # Almacenará la S.A. (Prompt 2)
...
with col_btn1:
    ...
    if st.button("Generar Plan de Foco Estratégico", use_container_width=True):
        ...
        # 1. Validación (Checkpoint Humano): ¿Ha rellenado el usuario lo básico?
        if not CE or not CEv or not TEMA:
            st.error("Por favor, rellena al menos el Tema, la Competencia Específica y el Criterio de Evaluación.")
        else:
            # 2. Mostrar un mensaje de "pensando..."
            with st.spinner("🤖 El Diseñador Jefe está creando tu Plan de Foco..."):

                # 3. Aquí es donde irá el PROMPT MAESTRO de tu compañero
                
                # --- (CORRECCIÓN 2: "Escapamos" {EsCE} y el JSON) ---
                ...
                # 5. Guardar el resultado en la "Memoria"
                #if respuesta_ia:
                st.session_state.plan_de_foco = respuesta_ia
                # --- AÑADIMOS UNA LÍNEA DE DEPURACIÓN ---
                # Mostramos la respuesta cruda en la propia app para verla
                st.write("Respuesta cruda de la IA (para depuración):")
...
# --- CUADRO DE TEXTO PARA LA SITUACIÓN DE APRENDIZAJE (SIN CAMBIOS) ---
