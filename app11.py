...
# ==============================================================================
# 🧠 FUNCIÓN DE CONEXIÓN CON REINTENTOS (EXPONENTIAL BACKOFF)
# Soluciona el error 429 (Resource Exhausted) esperando automáticamente.
# ==============================================================================
def stream_gemini_response(prompt_usuario, placeholder, model_name):
    ...
    max_retries = 3   # Intentos totales (1 normal + 2 reintentos)
    base_delay = 5    # Segundos de espera inicial

    # Instanciamos el modelo (el que hayas elegido en el selector)
    local_model = genai.GenerativeModel(model_name)

    # BUCLE DE VIDAS EXTRA
    for attempt in range(max_retries):
        try:
            ...
            for chunk in response:
                try:
                    if chunk.text:
                        ...
                        # Lógica del separador para ocultar el JSON mientras se genera
            ...
            # Si el código llega aquí, es que funcionó. 
            # Escribimos el texto final y salimos de la función.
            if not stop_updating_ui:
                placeholder.markdown(full_text)
                
            return full_text # ✅ ÉXITO ROTUNDO

        except Exception as e:
            error_str = str(e)
            
            # 🚦 DETECTOR DE TRÁFICO (Error 429)
            if "429" in error_str or "Resource exhausted" in error_str:
                if attempt < max_retries - 1: # Si nos quedan vidas...
                    
                    # Calculamos el tiempo de espera (5s -> 10s -> 20s)
                    wait_time = base_delay * (2 ** attempt) 
                    
                    # AVISAMOS EN PANTALLA (EN AMARILLO)
                    placeholder.warning(
                        f"🚦 **Tráfico alto en Google ({model_name}).** "
                        f"Pausando {wait_time} segundos para reintentar automáticamente... "
                        f"(Intento {attempt + 1} de {max_retries})"
                    )
                    
                    time.sleep(wait_time) # 😴 Python se duerme aquí
                    continue # Volvemos arriba al 'try'
                else:
                    # Se acabaron las vidas
                    placeholder.error(f"❌ La IA ({model_name}) está saturada y no admite más peticiones por ahora. Inténtalo en un minuto.")
                    return None
            else:
                # Si es otro error (ej. API Key inválida), fallamos inmediatamente
                placeholder.error(f"Error técnico al contactar con la IA ({model_name}): {e}")
                return None
...
if 'contador_sa' not in st.session_state: st.session_state.contador_sa = 0
    ...
    if not st.session_state.plan_json:
       ...
       5. Rúbrica de Evaluación
        ...
        if resultado_sa:
            ...
            # 🔥 SOLUCIÓN NUCLEAR: Cambiamos el ID para forzar el refresco
            st.session_state.contador_sa += 1 
            
            # Guardamos el modelo usado en sesión
...
if st.session_state.sa_generada:
    with contenedor_paso_4.container():
        ...
        with tab_editor:
            ...
            # Generamos una clave única cada vez que cambia el contador
            clave_dinamica = f"editor_sa_area_{st.session_state.contador_sa}"

            sa_final_editada = st.text_area(
                "Edita tu Situación de Aprendizaje:",
                value=st.session_state.sa_generada,
                height=600, 
                key=clave_dinamica, # <--- ¡CLAVE DINÁMICA!
                label_visibility="collapsed" 
            )
            
            # Sincronización manual: Si el usuario edita, actualizamos la variable maestra
